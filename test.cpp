...
int main()
{
	...
	score_win = create_newwin(hint_win_height, hint_win_width, 10, game_win_width+10);
	mvprintw(10, game_win_width+10+2,"%s","Score");
	refresh();
	...
	for(int i =0;i<4;i++)
		for(int j=0;j<4;j++)
			if(next_box_shape[i][j]==1){
				mvwaddch(hint_win,(hint_win_height-size_h)/2+i,(hint_win_width-size_w)/2+j,cc);
				wrefresh(hint_win);
			}
		...
		for(int i=0; i<size_h;i++)
			for(int j=0;j<size_w;j++){
				if(this->box_shape[i][j]==1){
					mvwaddch(game_win,head_y+i,head_x+j,cc);
					wrefresh(game_win);
				}
		}
